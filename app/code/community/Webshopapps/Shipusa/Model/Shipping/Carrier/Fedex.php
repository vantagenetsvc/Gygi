    ...
    protected function _formRateRequest($purpose)
    {
        ...
        if ($r->getOrigCountry() == self::USA_COUNTRY_ID && $r->getDestCountry() == self::CANADA_COUNTRY_ID) {
            $fedReq['RequestedShipment']['CustomsClearanceDetail']=array(
                'CustomsValue' => array('Amount'=>$r->getValue(),'Currency'=>'USD')
            );
        }
        ...
        if (is_object($response) && isset($response->RateReplyDetails) && $response->HighestSeverity != 'FAILURE'
            ...
            if (is_array($response->RateReplyDetails)) {
                foreach ($rateReplyDetails as $entry) {
                    ...
                    /**
                    * FedEx return both SHIPMENT and PACKAGE for single items. Shipment is for a group of ground items, package is for a single ground item.
                    */
                    if($entry->ServiceType == "FEDEX_GROUND") {
                        if($this->_numBoxes > 1){
                            $matchRateType = array($rateTypePre.'_'.$rateTypePost.'_SHIPMENT',$rateTypePre.'_'.$rateTypePost.'_PACKAGE');
                        } else {
                            $matchRateType = array($rateTypePre.'_'.$rateTypePost.'_PACKAGE');
                        }
                    }
    ...
    protected function fillOutputArrs($entry, $allowedMethods, $displayTransitTime,
        ...
        if (in_array((string)$entry->ServiceType, $allowedMethods)) {
            ...
            if (!is_array($entry->RatedShipmentDetails)) {
                foreach($matchRateType as $mrt) {
                    if (strpos((string)$entry->RatedShipmentDetails->ShipmentRateDetail->RateType,$mrt) !== false) {
                ...
                foreach ($entry->RatedShipmentDetails as $rateType) {
                    foreach($matchRateType as $mrt) {
                        if (strpos((string)$rateType->ShipmentRateDetail->RateType,$mrt) !== false) {
