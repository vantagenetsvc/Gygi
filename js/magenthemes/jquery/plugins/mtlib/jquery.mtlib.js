...
function filterCategory(catid,e){
    var url = "mtyoming/product/request/category/"+catid+"/";
    url += 'isAjax/1/';
...
function addCompare(e,url,id){
	url = url.replace("catalog/product_compare/add","mtyoming/product/compare");
	url += 'isAjax/1/';
...
function removeCompare(url){
	url = url.replace("catalog/product_compare/remove","mtyoming/product/rmcompare");
	url += 'isAjax/1/';
	$mtkb('#block-compare').addClass('compare-loading');
	$mtkb('#block-compare .block-content').css('display','none');
	$mtkb.ajax({
...
function clearCompare(url){
    url = url.replace("catalog/product_compare/clear","mtyoming/product/clearall");
    url += 'isAjax/1/';
    $mtkb('#block-compare').addClass('compare-loading');
    $mtkb('#block-compare .block-content').css('display','none');
    $mtkb.ajax({
...
function addWishlist(e,url,id){
	url = url.replace("wishlist/index","mtyoming/product");
	url += 'isAjax/1/';
...
function ajaxaddcart(url,id){
	...
	$mtkb('.cart').hide();
	data = '&isAjax=1&qty=1';
    url = url.replace("checkout/cart","mtyoming/index");
    try {
    	$mtkb.ajax( {
            url : url,
            dataType : 'json',
    ...
    $.extend(TwitterTimeline.prototype, {
        ...
        update: function(data) {
            ...
            $.each(data.reverse(), function(idx, item) {
                ...
                self.element.append(self.options.animateAdd($(tweet), idx));
